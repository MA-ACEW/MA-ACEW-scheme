...
//This is not our code, we only make some slight modifications to the code to make it work with our implementation.
// The source code is from https://github.com/sourav1547/wts.
import (
	"crypto/sha256"
	"encoding/binary"
	"fmt"
...
type Party struct {
	...
	pyKeyAff bls.G2Affine
	tpKeyAff bls.G1Affine
}
...
func (w *WTS) keyGenBench() {
	...
	// pKeyB :=  pKeyB.ScalarMultiplication(&w.crs.g1B, skInt)
...
// UpdateWeights updates the weights in the WTS system and recalculates related parameters
func (w *WTS) UpdateWeights(newWeights []int) error {
	// Verify if weight length matches number of signers
	if len(newWeights) != w.n {
		return fmt.Errorf("weight vector length(%d) does not match number of signers(%d)", len(newWeights), w.n)
	}

	// Update weight vector
	w.weights = newWeights

	// Recalculate preprocessing values related to weights
	weightsF := make([]fr.Element, w.n)
	for i := 0; i < w.n; i++ {
		weightsF[i] = fr.NewElement(uint64(w.weights[i]))
	}

	// Calculate weight commitment wTau
...
// Modify preProcess method, extract weight processing part
func (w *WTS) preProcess() {
	...
	// Use UpdateWeights function to handle weights
	_ = w.UpdateWeights(w.weights) // Using original weights here

	wg2.Wait()
	w.pp.qTaus = bls.BatchJacobianToAffineG1(qTaus)
}

// If weight-related values need to be calculated during WTS initialization, the constructor can also be modified
func NewWTS(n int, weights []int, crs CRS) WTS {
	w := WTS{
		n:       n,
		weights: weights,
	...
	// Process weights once during initialization
	_ = w.UpdateWeights(weights)
